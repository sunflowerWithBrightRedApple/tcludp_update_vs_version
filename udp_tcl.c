 ...
 * Copyright (c) 2024 by <ºt»~> <<fangql1713@outlook.com>>
 *
 * Written ºt»~
 * Last modified: 3/07/2024
 *
 * $Id$
 ******************************************************************************/
...
#include <winsock2.h>
#pragma comment(lib, "WS2_32")  // ¡¥Ω”µΩ WS2_32.lib
...
Udp_WinHasSockets(Tcl_Interp *interp)
{
    ...
    if (!initialized) {
        ...
        /*
         * Find out if we're running on win2000 or later.
         */
        ZeroMemory(&osvi, sizeof(OSVERSIONINFOEX));
        osvi.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEX);
        //info.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);
        // Set the version numbers to check for.
        ...
        /*
         * Check to see if Sockets are supported on this system.  atlest
         * win7, we need to look for it on the system first. 
        ...
        if (socketThread == NULL) {
            if (interp != NULL) {
                Tcl_AppendResult(interp, "Failed to create socket thread", NULL);
            }
            return TCL_ERROR;
        }
